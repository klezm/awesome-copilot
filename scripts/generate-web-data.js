...
function generateItems(dir, type, extension) {
  ...
  for (const file of files) {
    ...
    const link = encodeURI(`${type}/${file}`);
...
// Main execution
try {
  console.log("Generating web data...");

  const instructionsDir = path.join(__dirname, "../instructions");
  const promptsDir = path.join(__dirname, "../prompts");
  const chatmodesDir = path.join(__dirname, "../chatmodes");

  const instructions = generateItems(
    instructionsDir,
    "instructions",
    ".instructions.md"
  );
  const prompts = generateItems(promptsDir, "prompts", ".prompt.md");
  const chatmodes = generateItems(chatmodesDir, "chatmodes", ".chatmode.md");

  const allData = [...instructions, ...prompts, ...chatmodes];
  ...
  console.log("Web data generated successfully!");
} catch (error) {
  console.error(`Error generating web data: ${error.message}`);
  process.exit(1);
}
